...
export function StockDashboard({ data, loading, error }: StockDashboardProps) {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="grid gap-6">
                    ...
                    <div className="flex-1 rounded-3xl border border-border/50 bg-card/30 p-6 shadow-lg backdrop-blur-sm">
                        <div className="flex justify-between items-center mb-4">
                            <div className="flex items-center gap-2">
                                <h3 className="text-lg font-semibold">🎯 目標價與評級</h3>
                        ...
                        <div className="flex items-center justify-between gap-4">
                            <div className="text-center"><div className="text-sm text-muted-foreground mb-1">目標價</div><div className="text-2xl font-mono font-bold">{(data.targetPrice || 0).toFixed(2)}</div></div>
                            <div className="text-center">
                                <div className="flex items-center justify-center gap-1 mb-1">
                                    <div className="text-sm text-muted-foreground">技術評級</div>
                                    <InfoTooltip content={STOCK_DEFINITIONS["技術評級"]} />
                                </div>
            ...
            <div className="text-center text-xs text-muted-foreground opacity-50 pb-8">*SMC 指標與預測僅供參考，不代表投資建議。</div>
        </motion.div>
    )
}
